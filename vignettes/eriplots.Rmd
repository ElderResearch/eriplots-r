...
settings can be customized as needed. If you also need WebP output,
include `"webp"` in `formats` and `save_figures()` will use
[`ragg::agg_webp()`](https://ragg.r-lib.org/) (when available) to render
it. (And `ggplot2::ggsave()` will still work just fine, too.)
...
# Save a WebP copy (requires the ragg package)
save_figures("example.png", formats = c("webp"))
```
